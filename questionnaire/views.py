...
class QuestionView(View):
    ...
    def post(self, request):
        ...
        if (choice_id == 0):
            ...
            return self.render_invalid_post(request, question, 'Please select a choice')

        try:
            choice = question.choices.get(pk=choice_id)
        except ObjectDoesNotExist:
            return self.render_invalid_post(request, question, 'Please select a valid choice')
        ...
        if (request.user.is_anonymous):
            ...
            messages.add_message(request, messages.SUCCESS, 'Your answer was saved')
        else:
            # If it's a register user, requirements does not specify what to do
            messages.add_message(
                request, messages.INFO, 'You are a registered user, '
                'the questionnaire are only for guest users right now '
...
def restart(request):
    ...
    messages.add_message(request, messages.INFO, 'Please Redo Again')
    return redirect(reverse('index'))
